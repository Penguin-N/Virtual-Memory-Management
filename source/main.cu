...
// 32 kB in swap table
#define SWAP_PAGE_TABLE_SIZE (1 << 15)


//// count the pagefault times
__device__ __managed__ int pagefault_num = 0;
__device__ __managed__ int next_spare = 0;
...
// page table
extern __shared__ page_entry pt[];

// swap table
__device__ __managed__ s_page_entry st[SWAP_PAGE_TABLE_SIZE];
...
__global__ void mykernel(int input_size) {
  ...
  VirtualMemory vm;
  vm_init(&vm, data, storage, pt, st, &pagefault_num, &next_spare, PAGE_SIZE,
...
int main() {
  ...
  // write_binaryFile("./FM.bin", data, PHYSICAL_MEM_SIZE);
